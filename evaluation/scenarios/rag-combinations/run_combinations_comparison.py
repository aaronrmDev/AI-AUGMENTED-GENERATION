...
async def _run(combination: str) -> None:
    ...
    result = await use_case.execute(
        ...
        notes=(
            ...
            f"for the identical call: task_success_rate is now the average "
            f"across all repeat_count=3 samples per question (#147 raised "
            f"RunComparison from scoring only the last repeat to scoring "
            f"every one, project-wide), so a small amount of run-to-run "
            f"variance on borderline questions is expected and not itself "
            f"evidence of a methodology difference between combinations' "
            f"reports. CAVEAT "
            f"3: #148 (judge used to score both arms against only the "
            f"treatment's retrieved context) is now fixed -- each arm is "
            f"judged against its own context. For THIS script specifically, "
            f"baseline's own context is empty (CAVEAT 2), so baseline's "
            f"groundedness is now judged against '(none provided)': the "
            f"rubric scores that on whether baseline hedges appropriately "
            f"for having nothing to ground itself in, not as an automatic "
            f"floor -- but treat any baseline-vs-treatment groundedness gap "
            f"in this report as reflecting that structural difference in "
            f"what each arm had to work with, not a measured claim about "
            f"factual accuracy."
