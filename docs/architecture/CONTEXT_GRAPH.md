...
graph TD
    ...
    subgraph REAL_RAG["Module -> Class -> Test File (RAG, built)"]
        ...
        MOD_RAG_TEST["tests/unit/ (~30 files) + tests/integration/ (~15 files) --\n20 evaluation/reports/rag-*.md narrative reports"]
    end
...
composing the real evictors/compressors/offloaders below)"]
        MOD_CAG_INFRA["src/cag/infrastructure/ -- 35 classes:
...
6 multi-turn sessions, 2 allocators (paged, contiguous),
2 batch schedulers, 3 candidate generators,
VLLMCompletionClient + VLLMMetricsClient, HFTargetModel"]
        MOD_CAG_TEST["tests/unit/ (~40 files) + tests/integration/ (7 files;
5 need the real vLLM stack and skip outside it) --
8 evaluation reports under evaluation/reports/cag-*.md"]
    end
