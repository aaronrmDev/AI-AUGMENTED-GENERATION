...
**`paradigm_router.py`** — `decide(scores, select_threshold=0.5, uncertainty_margin=0.15) -> RoutingDecision`. Scores are independent per-paradigm values in [0, 1], because Concept 1's own table has multi-paradigm routes ("Compare today's sales with last month" → RAG + MAG) that a single-label classifier cannot express. Every paradigm scoring at or above the threshold is selected. The decision is confident only when no paradigm's score falls within the margin of the threshold on either side; when it is not, the decision widens to include every paradigm inside that band and switches to `PARALLEL` — Concept 1's "if classifier is uncertain, run parallel and merge", applied only to the paradigms genuinely in doubt. When no paradigm clears the threshold at all, the classifier has found no signal, which is uncertainty too, so `decide` returns `fallback_decision()`: every paradigm, in `PARALLEL`. An earlier draft picked the single highest score instead, and review showed what that did: a query with no signal was routed confidently to CAG alone, the one route where a weak frozen-cache match answers with nothing to contradict it. `fallback_decision()` is also the route `UnifiedAnswerQuestion` takes when the classifier times out, raises, or reports `ClassificationFailed`, so "the router could not decide" has exactly one meaning.
...
**`errors.py`** — `SessionNotFound`, raised when a budget record matches no session row; `QueryExceedsBudget`, raised when a question alone is larger than the Query slice; and `ClassificationFailed`, raised by a classifier that could not produce usable scores.
...
- `SemanticFactSearch.search(tenant_id, user_id, query_embedding, top_k)` is async, and each call is its own unit of work: the cascade cancels a timed-out tier mid-flight, and a cancelled query terminates the database connection it ran on, so a tier must never share a request's session.
- `SessionBudgetRecorder.record(tenant_id, user_id, session_id, allocation, contributing)` is async because it writes to Postgres.
...
- *`CagTier`* uses `CacheWarmedRetrieve`'s warmed-document match. That match is currently private and embeds the query itself; this batch extracts it as a public `best_warmed_match(tenant_id, query_embedding) -> SearchResult | None`, whose `score` carries the similarity, which keeps the existing FrozenCache content-hash confirmation intact. Because `CagTier` calls it on a worker thread, warmed entries are indexed by tenant and scanned from a snapshot, so `note_warmed` on the event loop can never change the dictionary mid-scan. `CacheWarmedRetrieve.execute` is rebuilt on top of the same method, so its behavior and its existing tests are unchanged. A score at or above `hit_threshold` is a `HIT`; a score at or above `partial_threshold` is a `PARTIAL`; anything lower is a `MISS`. The matching is CPU work, so it runs under `asyncio.to_thread`, which makes the 10ms timeout able to fire instead of being blocked by a stalled event loop.
- *`MagTier`* runs a `SemanticFactSearch` over the precomputed embedding. Its implementation, `SessionScopedSemanticFactSearch`, opens a short-lived session for every search, sets the tenant context inside it, and invalidates that session if the search fails or is cancelled. This came out of measurement and review: SQLAlchemy treats the cascade's cancellation as a disconnect and terminates the connection, after which `rollback()` and `close()` on that session both raise, so a tier sharing the request's session would take down every later use of it, including a Postgres-backed hybrid RAG retriever falling back after a MAG timeout. Invalidated and archived facts never reach it, because `PostgresSemanticMemoryRepository.search_by_similarity` already filters on `valid_until` and `archived_at`, which is Concept 5's "if MAG has stale state → invalidate". Facts scoring below `partial_threshold` are discarded. If the best remaining fact scores at or above `hit_threshold`, the result is a `HIT`; otherwise the remaining facts make a `PARTIAL`, and nothing remaining is a `MISS`.
...
2. Classify the query and decide the route, timed, under a `classifier_timeout`. A classifier that times out or raises yields `fallback_decision()`, and the answer reports which fallback happened in `routing_fallback`.
3. Run the cascade.
4. Allocate the budget from the cascade's `contributing` set.
5. Assemble the context.
6. If a `SessionBudgetRecorder` was supplied, record the allocation — before generation, so a session that doesn't exist fails the request before an answer is paid for.
7. Generate the answer with the RAG `ChatModel`.
...
- **`LlmQueryClassifier`** asks the `ChatModel` for a JSON object of per-paradigm confidences. If the response doesn't parse — the same failure #149 found in the project's own judge — it raises `ClassificationFailed` and counts the failure, and `UnifiedAnswerQuestion` maps that to `fallback_decision()`. An earlier draft returned 0.5 for every paradigm instead, which only lands inside the uncertainty band at the default threshold.
- **`PostgresSessionBudgetRecorder`** writes `{total, slices, contributing, recorded_at}` into `sessions.context_budget`. That JSONB column has existed since `alembic/versions/0001_users_sessions.py` and `docs/database/DATABASE.md` already describes it as "the per-session record of how the 128K context window gets sliced", so no migration is needed — this is the first code to use the column. The recorder opens and commits its own short transaction, calls `set_tenant_context` inside it so the table's `tenant_isolation` RLS policy enforces the tenant boundary, and matches the session on `user_id` as well as `id`, so one user cannot overwrite another user's record within the same tenant. An update that matches no row — a missing session, another tenant's, or another user's — raises `SessionNotFound` instead of passing silently.

## Tenant and user isolation

Every `TierRequest` carries both `tenant_id` and `user_id`, and every existing port a tier calls already enforces its own scope: `FrozenCache` and `CacheWarmedRetrieve` are tenant-scoped (a Batch D review fix), MAG semantic search is scoped by user and tenant, and the RAG retriever is tenant-scoped. No tier caches anything across requests. The one piece of shared state the batch adds sits outside the tiers: `CachingEmbeddingModel`, a single process-wide cache that `UnifiedAnswerQuestion` and the RAG retriever share, added after the first cascade measurement (see the plan's execution notes). An embedding is a pure function of its text, so a cached value reveals nothing a fresh one wouldn't, and entries are keyed by a SHA-256 digest, so the cache holds no query text. A hit is still faster than a miss, a signal that someone recently asked the exact same text. No HTTP endpoint exposes this path, so the signal can't be reached today; the security review of the endpoint that does must weigh it, and scoping the cache to a single request would remove it. The unit tests check that every tier passes the requesting tenant and user through to the store it calls. The integration tests check what the real stores then do: another user's MAG fact never reaches the tier, a MAG search under another tenant finds nothing, and a budget write under another tenant or another user is refused.
...
- `paradigm_router.decide` is tested against the score shapes of all six Concept 1 example queries, plus threshold boundaries, the no-signal fallback to every paradigm in `PARALLEL`, and the widening of `PARALLEL` to exactly the paradigms inside the uncertainty band.
...
- `LatencyCascade` has tests for:
  ...
  - access recording on a RAG hit, and that a failing tracker or findings sink never fails a request
  - background completion bounded by its deadline and its concurrency cap
  - a cancellation raised by one parallel tier cancelling its siblings
- The three tiers are tested for their threshold classification and scope propagation, and `CacheWarmedRetrieve`'s pre-extraction behavior tests still pass, joined by tests for the public match, its tenant scoping, and a document being warmed while a scan is in progress.
