...
(`docs/inputs/concepts/unified_rag_cag_mag_architecture.md`, Concept 3; CLAUDE.md §2.2)
...
(`docs/inputs/concepts/unified_rag_cag_mag_architecture.md`, Concept 5; CLAUDE.md §2.3)
...
The stack below is what CLAUDE.md's existing §3.1–§3.8 already commits to, carried over here with one correction: React is **18+** throughout. CLAUDE.md's current header for its frontend section reads "React 19 Atomic Design," but its own stack table in the very same section lists "React 18+" — and neither source document mentions React 19 anywhere. That's an internal contradiction in the original file, not a deliberate version choice, and it's corrected here to React 18+ consistently, since this is where that table now lives. Five of the choices below already have their reasoning recorded separately as ADRs — vLLM over SGLang, Qdrant over Milvus/Weaviate, standard attention over alternative attention, Hexagonal+CQRS for MAG, and LangChain/LangGraph for RAG orchestration — in `docs/decisions/adr/`; this section is the inventory, those files are the "why."
...
**If you'd rather build in Go or Rust:** FastAPI's rough equivalents are Gin or Echo in Go and Axum or Actix in Rust; Celery's nearest Go equivalent is Asynq, with Rust offering no direct match beyond building on `tokio` directly; Qdrant and PyTorch both have official or partial client bindings in both languages, though PyTorch's Rust binding (`tch-rs`) is explicitly called out as limited. The source's own verdict is blunt about when this is worth it: Python is strongly recommended for this project because of its ML ecosystem, and Go or Rust make sense only for specific, narrowly-scoped high-performance microservices carved out of the larger system — not as a wholesale replacement for the Python stack above.
...
`fullstack_unified_ai_system.md` §4 lays out a concrete `src/` module structure for this system, organized around the same three paradigms plus the orchestration layer that coordinates them. **None of the directories below exist in this repository as of this writing.** There is no `src/`, no `tests/`, no `docker/`, no `k8s/` — this project is documentation-only at this stage, and creating empty scaffolding for code that doesn't exist yet would just be a different version of the "structure without substance" problem this documentation effort is meant to avoid. What follows is the blueprint this project commits to building toward, and it becomes real specifically when Phase 1 implementation begins — not before, and not as a side effect of writing this document.
