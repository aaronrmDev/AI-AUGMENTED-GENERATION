...
- [docs/architecture/CAG.md](docs/architecture/CAG.md) — the nine CAG techniques mapped onto the serving pipeline: eviction, compression, offloading, speculative decoding, and the attention-compatibility split ADR-0003 establishes; six of the nine (alternative-attention compatibility, KV cache compression, speculative decoding, KV cache eviction, prefix caching, and PagedAttention) are built and live-measured, the remaining three queued for implementation against a serving stack that is already confirmed working.
