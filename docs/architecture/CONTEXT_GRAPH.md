...
8 evaluation reports under evaluation/reports/cag-*.md"]
    ...
    subgraph FUTURE["Module -> Class -> Test File (genuinely still unbuilt)"]
        MOD_ORCH_META["src/orchestration/ -- freshness_router.py (the Freshness-Aware\nData Router: ingestion-time routing, the one meta-layer coordinator\nstill unbuilt) -- plus an HTTP endpoint exposing UnifiedAnswerQuestion"]
...
Three things remain genuinely unbuilt. The first is the Freshness-Aware Data Router (`freshness_router.py`), now the only orchestration meta-layer coordinator without code. It decides once per data source, at ingestion, which paradigm should own that data, which is a different moment and a different caller from the per-query decisions the Paradigm Router, the Context Budget Allocator, and the Latency-Adaptive Fallback Cascade make; those three moved out of this section and into `REAL_ORCH` in this generation. An HTTP endpoint exposing `UnifiedAnswerQuestion` is deliberately unbuilt too, because it would accept a client-supplied session and needs its own object-level authorization review. The second and third are the frontend (React 18+, Tailwind, Zustand, SSE/WebSocket streaming) and the background-worker/Kubernetes/production-deployment layer (`src/workers/`, `k8s/`, `docker-compose.prod.yml`, the experiment notebooks), both named in the Phase 1 blueprint and both entirely unbuilt.
...
Class remains the one level this diagram represents as detail-within-a-Module-node rather than a fully separate rank, the same resolution choice carried over from the first generation: with 256 class definitions now existing across `src/` alone (counted with Python's standard-library `ast` module in this generation, the same check used to confirm the per-layer counts in `REAL_ORCH`), drawing one node per class would produce an unreadable diagram at this diagram's chosen scale. CLAUDE.md's convention names five levels; this diagram has always represented Class as a label detail rather than a node, for every context it draws real code for.

## Keeping this current

This diagram is generated output, not something to hand-edit when a new doc or module gets added — re-running `graphify` and redrawing the five-level structure above is the intended way to refresh it. This fifth generation, triggered by the per-query orchestration meta-layer landing, re-ran `graphify`'s AST-based structural extraction against `src/` (213 code files, 1354 nodes, 3522 raw edges, deterministic parsing with no LLM cost, from a second extraction after the batch's measurement pass added `CachingEmbeddingModel` to RAG's infrastructure) and redrew the `REAL_ORCH` subgraph from what that extraction reported, cross-checked against a standard-library `ast` count: 26 domain classes, 19 application classes, and 11 infrastructure classes. The fourth generation did the same for CAG alone, and the third for RAG, CAG, MAG and Orchestration at once; contexts not redrawn here are unchanged because their code is. Verifying against the repository rather than against a prior session's memory of what was built is the same discipline `docs/governance/AUTOLEARNING.md`'s `claude-md-sync` skill applies to CLAUDE.md and the rest of `docs/`, and which now exists as a real, working skill rather than the "doesn't exist yet as of this writing" caveat the second generation carried. The next trigger for a sixth generation is named directly in "What isn't here yet" above: the Freshness-Aware Data Router landing, or the frontend and worker layers starting -- any of which would move a `FUTURE` node into a `REAL_*` subgraph the way this generation did for the per-query orchestration meta-layer.
