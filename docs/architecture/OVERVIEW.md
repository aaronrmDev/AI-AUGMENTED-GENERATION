...
The API composes the cascade with MAG and RAG tiers only (`src/api/unified_pipeline.py`). A CAG tier needs a warmed frozen cache in the serving process and a worker to warm it, and neither exists yet, so a query the router sends to CAG alone is answered from RAG. Answering shares a per-user rate limit with the RAG-only `POST /chat`, and the streaming endpoint shares that same budget too rather than getting a second one. The batch's live run against uvicorn and Ollama, and its security review, are recorded in `docs/superpowers/plans/2026-09-13-unified-api-sessions.md`; the streaming variant's own design is in `docs/superpowers/specs/2026-09-17-streaming-answers-design.md`, and its security review is recorded in this feature's SDD ledger and PR history.
...
No frontend exists in this repository yet, and no framework, styling, or state-management library has been chosen for it. `fullstack_unified_ai_system.md` §3.8 proposed one candidate stack, but that was the source material's own suggestion, not a decision this project has ratified — the actual choice is deferred until frontend work begins.
...
### `src/workers/` is this project's first background-worker subsystem, and it's real

The tree diagram above still shows the blueprint's originally-imagined `src/workers/` — four separate task modules for consolidation, RAG indexing, cache warmup, and cross-paradigm sync, none of which exist. What actually landed is narrower and serves one real caller: `POST /data-sources` (`src/api/routers/data_sources.py`) accepts a freshness-routed ingestion request and returns `202 Accepted` with a task id rather than blocking the request on `IngestDataSource.execute()`, and `GET /data-sources/jobs/{task_id}` polls for the result. `src/workers/celery_app.py` builds the Celery application (`build_celery_app()`, exported as the module-level `celery_app`) with Redis as both broker and result backend, and `JOB_TTL_SECONDS` (24 hours) bounding how long a result — and the ownership record described below — stays queryable. `src/workers/celery_ingestion_dispatcher.py`'s `CeleryIngestionJobDispatcher` is the real `IngestionJobDispatcher` (`src/orchestration/domain/ports.py`) the router depends on: `dispatch()` sends the Celery task and writes a `tenant_id:user_id` ownership record into Redis (`docs/database/DATABASE.md` has the key shape), and `status()` checks that record before ever reading Celery's own result backend, so a task id belonging to another tenant or user reports back exactly as if it never existed. `src/workers/ingestion_worker.py`'s `ingest_data_source_task` is the actual Celery task, and it's also its own composition root: since a worker process has no request-scoped dependency injection the way `src/api/dependencies.py` gives the API, this module builds a fresh Postgres engine, Qdrant-backed vector store and semantic index, and Neo4j driver on every invocation, backing `IngestDataSource` with `NullFrozenCache` (`src/orchestration/infrastructure/null_frozen_cache.py`) — a disclosed no-op stand-in for the CAG cache no production process serves yet, so ingestion's cache-touching code paths do real, harmless nothing rather than reaching for infrastructure that isn't there. All three modules are tested against real infrastructure, not mocks: `tests/integration/test_celery_ingestion_dispatcher.py` and `tests/integration/test_ingestion_worker.py` cover the dispatcher and the task in isolation, and `tests/integration/test_data_sources_endpoints.py` starts a real Celery worker via `celery.contrib.testing.worker.start_worker` against a TestContainers-provisioned Redis broker and proves a real end-to-end ingestion through HTTP, against real Postgres and Qdrant, task id to stored chunk.
