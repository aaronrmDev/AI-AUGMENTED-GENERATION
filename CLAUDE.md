...
Domain → Bounded Context → Module → Class → Test File is meant to be mapped as a Mermaid diagram maintained via `graphify`, so the relationships between this codebase's pieces are queryable instead of something every session reconstructs from scratch by re-reading source files. That diagram is generated output, and it now lives at [docs/architecture/CONTEXT_GRAPH.md](docs/architecture/CONTEXT_GRAPH.md), now in its fifth generation: real Module, Class, and Test File nodes exist for every bounded context with real code behind it — RAG, CAG, MAG, Orchestration (including the per-query Paradigm Router, Context Budget Allocator, and Latency-Adaptive Fallback Cascade), Identity & Access, and the Comparison & Measurement Infrastructure context for the evaluation harness — verified directly against the repository via `graphify`'s own AST extraction rather than recalled from memory. Only the Freshness-Aware Data Router, an HTTP endpoint for the unified per-query path, and the frontend and worker layers remain the dashed, genuinely-unbuilt nodes the diagram's own "What isn't here yet" section names directly. [docs/governance/SKILL_ROUTING.md](docs/governance/SKILL_ROUTING.md) covers when to reach for `graphify` beyond a full regeneration like this one.
