...
8 evaluation reports under evaluation/reports/cag-*.md"]
    ...
    subgraph FUTURE["Module -> Class -> Test File (genuinely still unbuilt)"]
        MOD_ORCH_META["src/api/ -- HTTP endpoints for the orchestration meta-layer: the\nunified per-query path and freshness-routed ingestion (all five\ncoordinators are built; each endpoint needs its own authorization review)"]
...
Three things remain genuinely unbuilt. The first is HTTP access to the orchestration meta-layer. All five of its coordinators are built: the Freshness-Aware Data Router moved into `REAL_ORCH` in this generation, following the three per-query components the fifth generation moved. But no endpoint exposes `UnifiedAnswerQuestion` or freshness-routed ingestion, because each would accept client-supplied scope and needs its own object-level authorization review. The second and third are the frontend (React 18+, Tailwind, Zustand, SSE/WebSocket streaming) and the background-worker/Kubernetes/production-deployment layer (`src/workers/`, `k8s/`, `docker-compose.prod.yml`, the experiment notebooks), both named in the Phase 1 blueprint and both entirely unbuilt.
