...
class HFFrozenCache(FrozenCache):
    ...
    talks to a real model. `tokenizer`/`model` can be passed in to reuse an
    already-loaded pair (this project's own session-scoped
    ...
    seam; production code just calls `HFFrozenCache()`.
    ...
    def __init__(
        ...
        self._model.eval()
        self._entries: dict[uuid.UUID, tuple[str, Any]] = {}
    ...
    def preload(self, document_id: uuid.UUID, content: str) -> None:
        ...
        self._entries[document_id] = (content_hash(content), output.past_key_values)
    ...
    def prefill_latency_ms(self, document_id: uuid.UUID, content: str) -> tuple[float, float]:
        ...
        be preloaded.
        """
        cached_hit = self.lookup(document_id)
