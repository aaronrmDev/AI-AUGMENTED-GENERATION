...
Two models are meant to be served locally against this hardware once vLLM-on-ROCm serving is actually stood up, and three more are meant to be called through their APIs as reference points rather than self-hosted — the full reasoning for why each one is in which category, and the citations behind every figure below, live in `docs/evaluation/COMPARISON_METHODOLOGY.md`; this table is the inventory, that document is the "why." That ROCm serving setup itself hasn't happened yet, and it's the one piece of this project's roster that needs the machine's own owner present for host-level driver work rather than something a session can stand up unsupervised — every batch of real, measured work this project has produced so far (every evaluation report under `evaluation/reports/`) used CPU-feasible substitutes instead: Ollama, real small Hugging Face models where a technique's own correctness genuinely depends on a real forward pass (`distilgpt2` throughout CAG's KV-cache-compression and speculative-decoding batches), and `sentence-transformers` for embeddings. The GPU-and-vLLM-dependent CAG techniques named throughout `docs/architecture/CAG.md` and the Phase 1 blueprint below are exactly the pieces of work still queued behind this specific setup step.
