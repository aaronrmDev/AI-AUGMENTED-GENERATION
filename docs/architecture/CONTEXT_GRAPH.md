...
besides cag-mag-synthesis.md"]
    ...
    subgraph FUTURE["Module -> Class -> Test File (genuinely still unbuilt)"]
        MOD_ORCH_META["src/api/ -- an HTTP endpoint for freshness-routed ingestion\n(routers/sessions.py already serves the unified per-query path;\ningestion needs a tenant role model and its own authorization review)"]
...
Three things remain genuinely unbuilt. The first is HTTP access to freshness-routed ingestion. All five of the orchestration meta-layer's coordinators are built: the Freshness-Aware Data Router moved into `REAL_ORCH` in this generation, following the three per-query components the fifth generation moved. Since then, `src/api/routers/sessions.py` has exposed `UnifiedAnswerQuestion` behind an object-level ownership check on each session (Story #183). But no endpoint exposes freshness-routed ingestion, which accepts client-supplied scope and needs a tenant role model before its own authorization review. The second and third are the frontend (no framework chosen yet) and the background-worker/Kubernetes/production-deployment layer (`src/workers/`, `k8s/`, `docker-compose.prod.yml`, the experiment notebooks), both named in the Phase 1 blueprint and both entirely unbuilt.
