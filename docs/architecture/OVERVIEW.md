...
This tiering boundary is built and live-measured across all three pairings this project has a boundary for — the only one of the three named synthesis techniques that's true of. RAG+CAG's `TieringPolicy`/`WarmCache`, RAG+MAG's `MagTieringPolicy`, and CAG+MAG's `CagMagTieringPolicy` each implement the promote/demote decision for their own pairing, all three tested against a real `HFFrozenCache` — a genuine transformer KV cache, not a stub — with real measured time-to-first-token (TTFT) wins ranging from roughly 1.5x to 2.6x depending on the pairing and content source (`evaluation/reports/rag-cag-synthesis.md`, `rag-mag-synthesis.md`, `cag-mag-synthesis.md`).
...
Two models are meant to be served locally against this hardware, and three more are meant to be called through their APIs as reference points rather than self-hosted — the full reasoning for why each one is in which category, and the citations behind every figure below, live in `docs/evaluation/COMPARISON_METHODOLOGY.md`; this table is the inventory, that document is the "why." The vLLM-on-ROCm serving stack itself is confirmed set up and working (ROCm 7.2.0, vLLM 0.28.0, verified with a real generation and zero GEMM errors), but no batch of this project's real, measured work has used it yet — every evaluation report under `evaluation/reports/` so far used CPU-feasible substitutes instead: Ollama, real small Hugging Face models where a technique's own correctness genuinely depends on a real forward pass (`distilgpt2` throughout CAG's KV-cache-compression and speculative-decoding batches), and `sentence-transformers` for embeddings. The GPU-and-vLLM-dependent CAG techniques named throughout `docs/architecture/CAG.md` and the Phase 1 blueprint below are exactly the pieces of work that now have a working serving stack to build and measure against.
