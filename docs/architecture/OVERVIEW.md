...
`fullstack_unified_ai_system.md` §4 lays out a concrete `src/` module structure for this system, organized around the same three paradigms plus the orchestration layer that coordinates them. The Auth Foundation sub-project (August 2026) has established the authentication infrastructure alongside these paradigm modules: `src/api/` and `src/identity/` now exist, along with `tests/unit/`, `tests/integration/`, and the Docker configuration needed to run them locally (`docker/Dockerfile.api` and `docker/docker-compose.yml`). The three paradigm modules themselves — `src/rag/`, `src/cag/`, `src/mag/` — remain pending, as do `src/orchestration/`, `src/workers/`, the Kubernetes configuration under `k8s/`, the additional Docker images (`Dockerfile.worker` and `Dockerfile.vllm`) and production Docker Compose variant (`docker-compose.prod.yml`), and the experiment notebooks under `notebooks/`. The blueprint below documents the full target structure; what follows is both a record of what's been built so far and a guide for what remains to be built.
