...
- [docs/architecture/CAG.md](docs/architecture/CAG.md) — the nine CAG techniques mapped onto the serving pipeline: eviction, compression, offloading, speculative decoding, and the attention-compatibility split ADR-0003 establishes; all nine are built and measured, four of them validated against a live vLLM server and the rest compared in simulation where the subject allowed nothing else against a serving stack that is already confirmed working.
