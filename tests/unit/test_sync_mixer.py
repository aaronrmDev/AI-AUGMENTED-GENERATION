...
def test_no_cached_entry_means_nothing_to_reconcile():
    result = reconcile(cached_hit=None, authoritative_content="anything", document_id=uuid.uuid4())
    assert result is None


def test_matching_content_is_already_in_sync():
    content = "return policy: 30 days"
    hit = CacheHit(content_hash=content_hash(content), kv_cache=object())

    result = reconcile(cached_hit=hit, authoritative_content=content, document_id=uuid.uuid4())
...
def test_mismatched_content_is_a_conflict_and_rag_wins():
    ...
    hit = CacheHit(content_hash=content_hash(stale_content), kv_cache=object())
    document_id = uuid.uuid4()

    result = reconcile(cached_hit=hit, authoritative_content=fresh_content, document_id=document_id)
