...
Class remains the one level this diagram represents as detail-within-a-Module-node rather than a fully separate rank, the same resolution choice carried over from the first generation: with 272 real classes now existing across `src/` alone (confirmed by this generation's own AST extraction), drawing one node per class would produce an unreadable diagram at this diagram's chosen scale. CLAUDE.md's convention names five levels; this diagram has always represented Class as a label detail rather than a node, for every context it draws real code for.

## Keeping this current

This diagram is generated output, not something to hand-edit when a new doc or module gets added — re-running `graphify` and redrawing the five-level structure above is the intended way to refresh it. This fourth generation, triggered by CAG completing -- seven batches taking it from three of nine techniques to all nine plus its three combination archetypes -- re-ran `graphify`'s own AST-based structural extraction directly against `src/` (200 code files, 1221 nodes, 3569 edges, deterministic parsing with no LLM cost) and redrew the `REAL_CAG` subgraph from what that extraction actually reported: 23 domain classes, 35 infrastructure classes, 5 application classes. The third generation did the same for RAG, CAG, MAG and Orchestration at once against 167 files; those other contexts' nodes are unchanged here because their code is. Verifying against the repository rather than against a prior session's memory of what was built is the same discipline `docs/governance/AUTOLEARNING.md`'s `claude-md-sync` skill applies to CLAUDE.md and the rest of `docs/`, and which now exists as a real, working skill rather than the "doesn't exist yet as of this writing" caveat the second generation carried. The next trigger for a fifth generation is named directly in "What isn't here yet" above: the orchestration meta-layer's own router/allocator/cascade files landing, or the frontend and worker layers starting -- either of which would move a `FUTURE` node into a `REAL_*` subgraph the same way this generation did for the CAG context and the third did for RAG, CAG, MAG, and Orchestration together.
