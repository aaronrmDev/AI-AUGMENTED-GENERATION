...
**Postgres** (`postgres_episodic_memory_repository.py`, `postgres_semantic_memory_repository.py`): raw SQL via `text()`, matching `PostgresUserRepository`'s existing convention — no ORM. Row-level security follows the `tenant_isolation` policy pattern from migration `0002`: both `EpisodicMemory` and `SemanticMemory` get an explicit `tenant_id` column (mirroring `Chunks`, since MAG rows are read/written per-request and the RLS policy needs to evaluate directly against the row without a join, same reasoning `DATABASE.md` gives for `Chunks.tenant_id`).
>
> **Correction, post-review:** this section originally scoped `SemanticMemory` through `user_id` alone, with no RLS, on the claim that this "matches how `Sessions` scopes today." That claim was false — `Sessions` carries its own `tenant_id` column and RLS policy from migration `0001`; nothing in this schema was ever scoped through an indirect foreign-key join alone. The adversarial review that caught this also traced the consequence directly: with no RLS backstop, an application bug (a forgotten `WHERE user_id = :user_id`) would leak every tenant's facts with nothing in the database to stop it — exactly the gap `docs/security/SECURITY.md`'s RLS-everywhere control exists to close. `SemanticMemory` now gets the same direct `tenant_id` + RLS treatment as `EpisodicMemory`, and `fact_key` gained a `UNIQUE (user_id, fact_key)` constraint (a separate, also-review-caught defect: `RecordSemanticFact` is meant to upsert a fact, and a non-unique index let two calls with the same key produce two rows resolved nondeterministically at read time).
...
New Alembic migration `0003_mag_episodic_semantic_memory.py`, revises `0002`. Creates `episodic_memory` (id, session_id → sessions.id, tenant_id, content JSONB, embedding vector(384), timestamp, salience_score) and `semantic_memory` (id, user_id → users.id, tenant_id, fact_key, fact_value, confidence, source, valid_until nullable, embedding vector(384)), with RLS enabled on **both** tables (direct `tenant_id` column, same `tenant_isolation` policy as `chunks`) and a `UNIQUE (user_id, fact_key)` constraint on `semantic_memory` (see the correction note above the Infrastructure section for why both differ from the batch's first-pass version). `GRANT` matches `app_user`, matching migration `0002`'s convention.
...
> **Correction, post-review:** the live measurement this section predicts a Redis win did not hold once measured correctly. A first-pass version of the benchmark reported a 1.3–1.4× Redis speedup, but that number was produced under a broken `ANALYZE` call that silently never updated Postgres's table statistics (the application's `app_user` role lacks the privilege, confirmed empirically); once fixed, four independent runs showed Postgres at or slightly *faster* than Redis at this benchmark's scale (12,000 background rows). See `evaluation/reports/mag-foundation.md`'s Memory Hierarchy section for the full, retracted-and-corrected account. The architectural reasoning above is still the right prediction in principle — it just isn't what this benchmark's scale actually demonstrated, and the report says so rather than keeping the original number.
