...
The third group is the RAG document pipeline: `Documents` tracks an uploaded file's status and chunk count, and `Chunks` holds the actual pieces that get embedded and retrieved. `Chunks.parent_id` is a self-reference — it's what makes parent-document retrieval possible, where a small chunk is what matches a query but a larger surrounding block is what actually gets handed to the model.

None of the three groups repeats a `tenant_id` column explicitly except `Users`, `Sessions`, and `Documents` — the tables shown below. The rest scope to a tenant indirectly, through the foreign key that ties them back to a `Sessions` or `Users` row that does carry one; row-level security policies enforce the filter at the point where it actually matters; whether the intermediate tables also need their own `tenant_id` column for performance is a detail this document doesn't resolve on its own.
...
## Qdrant holds the vectors that need fast nearest-neighbor search at scale

Every embedding in this system could, in principle, live only in the `pgvector` columns already sitting in PostgreSQL — the previous section showed three tables that do exactly that. Qdrant exists anyway because there's a real difference between storing a vector next to its row of truth and running fast approximate nearest-neighbor search over the whole corpus of vectors at once. `pgvector` earns its place by keeping an embedding transactionally consistent with the row it belongs to; Qdrant earns its place by being a database built from the ground up to answer "what are this vector's nearest neighbors" quickly even as that corpus grows into the millions, which was the specific reason this project chose Qdrant over Milvus and Weaviate as its vector store (`docs/decisions/adr/0002-qdrant-over-milvus.md`). In practice that means the same data effectively gets written twice on purpose: once into PostgreSQL as the durable, relationally-linked record, and again into a Qdrant collection tuned purely for retrieval speed.
