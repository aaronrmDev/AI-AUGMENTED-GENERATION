...
Executed inline via `superpowers:subagent-driven-development`, one fresh implementer subagent per task plus a dedicated task reviewer, a fix loop where a review found issues, and a final whole-branch review with one fix wave. Full ledger: `.superpowers/sdd/2026-09-15-unified-api-hardening/progress.md` (deleted with the workspace after the final review passed; the commits below are the durable record).

**Departures from the plan as written:**
- Task 1's implementer found, mid-implementation, that the brief's literal wiring order (per-user check, then global check) broke a pre-existing test through a header-overwrite interaction in `enforce_rate_limit`, and initially fixed it by reversing the enforcement order. The task reviewer caught that this reopened exactly the DoS vector Finding 1 exists to close: `RedisRateLimiter.check()` increments its counter on every call regardless of allow/deny, so a global-first order let an account already over its own per-user limit still burn the shared global quota on its way to rejection. Fix round 1 reverted the order and instead fixed the header-overwrite bug at its actual source (a "first successful call's headers win" guard), verified by a new test reading the Redis counter directly.
- The final whole-branch review found that this plan's own Step 7 wiring (per-user check first, global check second, `_DEFAULT_GLOBAL_CHAT_LIMIT = 1000`) left the global quota's default *below* a single user's own theoretical per-hour maximum (100/minute sustained = 6,000/hour), which would make the shared quota bind on one well-behaved account before it ever stopped an attacker spread across many. Ruled and fixed in the final fix wave: raised to `10000` — roughly 10x a single user's own max, chosen to comfortably cover several concurrently active tenants while still bounding aggregate registration-driven cost to a fixed number per hour. `docs/security/SECURITY.md` and the design spec were updated to the new value in the same pass; this plan's own Step 7 snippet and Task 6's dispatch context above still show the original `1000` figure, left as the historical record of what was planned rather than edited to match, per this project's convention for `docs/superpowers/` content.
- The final review also found the `/documents` body-size override was a path-prefix match, so `/documents/search` — a pure-JSON, unrate-limited route with no size bound of its own — silently inherited the 11 MiB upload ceiling instead of the 16 KiB default. Fixed by switching `_limit_for` to exact-path matching.
- Ten further minor findings from the final review (dead code, a fragile header-invariant gate, an import-time side effect, header-parsing robustness, a malformed-env-var crash path, a stale doc sentence, and two test-accuracy nits) were fixed in the same fix wave; one (duplicating the global-quota wiring block between `chat.py` and `sessions.py`) was deliberately left as-is, since the codebase already duplicates the pre-existing per-user check the same way, and abstracting only the newer duplication would have been the inconsistent choice.

**Commits (`feature/193-unified-api-hardening`, `9b1790e..30ce9a4`, 11 commits):** the design spec (`9b1790e`); the plan itself (`f0a0c8a`); Task 1's global chat quota plus its fix round (`396a213`, `7449ff9`); Task 2's body-size middleware (`4e31ef6`); Task 3's trusted-proxy client IP (`433ea71`); Task 4's security-header middleware (`01e0048`); Task 5's structured security-event logging (`91cf7da`); Task 6's documentation and final verification (`a432f99`); the final-review fix wave (`30ce9a4`).
