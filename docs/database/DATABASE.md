...
| Sessions | user_id | foreign key → Users; indexed with `created_at DESC` (migration 0007) |
| Sessions | tenant_id | — |
| Sessions | title | — |
...
One key pattern from that original concept is real and built: `session:{session_id}:working_memory` is the current turn's live context — the fastest tier of MAG's memory hierarchy, read and written on nearly every request, implemented by `RedisWorkingMemoryStore` (`src/mag/infrastructure/redis_working_memory_store.py`). The other three items the concept material described for this store — `user:{user_id}:preferences`, a `cag:prefix:{hash}` prefix-cache bookkeeping key, and a `memory:updates` pub/sub channel for cross-service invalidation — were never built: no file under `src/` writes or reads any of those three, and this document previously described them as though they were. What actually replaced that pub/sub design is simpler and lives at the orchestration layer instead of in Redis: `src/orchestration/domain/sync_mixer.py`'s `reconcile()` function compares a content hash the caller already holds against the authoritative source's current hash, synchronously, inside whichever service already has both values — no broadcast, no subscriber, no Redis channel in between. `docs/architecture/OVERVIEW.md`'s "sync mixer" section describes that mechanism directly; this document no longer claims a Redis channel implements it.

Auth Foundation actually built two real stores on Redis instead, each with its own key prefix — something this document didn't previously mention at all: `RedisRefreshTokenStore` (`src/identity/infrastructure/redis_refresh_token_store.py`) holds refresh-token state under `identity:refresh:{token_id}` so a token can be revoked or rotated without a database round trip, and `RedisRateLimiter` (`src/identity/infrastructure/redis_rate_limiter.py`) counts requests per fixed window under `identity:ratelimit:`, keyed by client IP for registration and login (`register:{client_ip}`, `login:{client_ip}`), by user for `POST /chat` and session answers, which share `chat:{user_id}`, by user for session creation (`sessions:{user_id}`), and by user for `POST /data-sources` (`data-sources:{user_id}`, `INGESTION_RATE_LIMIT`, `src/api/rate_limit.py`). Neither store's keys include a tenant id. Both are covered by real integration tests against a TestContainers-provisioned Redis (`tests/integration/test_redis_refresh_token_store.py`, `test_redis_rate_limiter.py`).

The freshness-routed ingestion endpoint's background-worker subsystem added a third: `CeleryIngestionJobDispatcher` (`src/workers/celery_ingestion_dispatcher.py`) writes `ingestion_job:{task_id} -> "{tenant_id}:{user_id or ''}"` in `dispatch()`, at the same moment it hands the ingestion off to Celery, with a 24-hour TTL (`JOB_TTL_SECONDS`, `src/workers/celery_app.py`) — the same lifetime as the Celery result backend's own `result_expires`, so neither the ownership record nor the result it gates outlives the other. `status()` reads this key before it ever reads Celery's result backend: a task id with no key, or one whose tenant or user doesn't match the caller, is reported back exactly as though it never existed, which is what lets `GET /data-sources/jobs/{task_id}` give an unrelated caller the same 404 a missing task id would (`docs/security/SECURITY.md` covers the authorization shape). It's covered by `tests/integration/test_celery_ingestion_dispatcher.py`, and end to end through the running app and a real Celery worker by `tests/integration/test_data_sources_endpoints.py`.
...
| Rate-limiter store (own key prefix) | Fixed-window request counts per client IP or per user | Built — `RedisRateLimiter`, not part of the original concept material |
| `ingestion_job:{task_id}` -> `"{tenant_id}:{user_id or ''}"` | Ownership record gating who may poll an ingestion job's status, TTL 24h | Built — `CeleryIngestionJobDispatcher`, not part of the original concept material |
...
| Indexes | `Entity.name`, `Entity.embedding` (created, but no embedding is written yet) |
